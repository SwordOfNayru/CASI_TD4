...
import com.casi_td4.Factory.DAOFactory.Factories;
import com.casi_td4.modele.Banque;
import com.casi_td4.modele.BanqueFacade;
...
public class BanqueView extends JFrame {
     ...
     */
    BanqueFacade facade = new BanqueFacade(DAOFactory.getFactory(Factories.JSON));
    ...
    private void chargerCompteSelectionne() {
        ...
        if (comboComptes.getSelectedItem() != null) {

            Compte c = (Compte) comboComptes.getSelectedItem();

            labelCompte.setText(c.getNumCompte());
            labelSolde.setText(String.valueOf(c.getSolde()));

            listmodelOperations.addAll(c.getLesOperations());
        }

        /*
         * Remplir les JtextField du compte avec le compte s�lectionn�
         * Remplir la JList des op�rations. Pour cela, utiliser ici la m�thode
    ...
    private void ajouterCompte() {
        ...
        if (jd.isSaisieOK()) {
            /*
             * Cr�er le nouveau compte avec les donn�es de la JDialog
             */
            c = facade.ajouterCompte(jd.getNumeroCompte(), jd.getSolde());
            facade.saveAll();
            comboComptes.addItem(c);
        }
    }
    ...
    private void ajouterOperation() {
        ...
        jd.setVisible(true);
        if (jd.isSaisieOK()) {
            facade.ajouterOperation(jd.getIntitule(), jd.getDate(), jd.getMontant(), compteSelectionne);
            facade.saveAll();
            chargerCompteSelectionne();
        }
    }
